...
object ClientHello extends App {
...
class ConnActor(httpClient: ActorRef,
  ...
  val connected: Receive = ({
    case Ping =>
      log.debug("Connection {}: sending Hello", connNr)
      reqCounter += 1
  ...
  val responsePending: Receive = ({
    ...
    case HttpResponse(200, _, body, _) =>
      log.debug("Connection {}: Hi!", connNr)
  ...
  def onResponse(response: String) {
    if (response == "Hi!") {
      ...
      log.error("Received incorrect response '{}', should have been '{}'", response)
      context.system.shutdown()
    }
  }
